...
function q_constitution() {
    q(q_privateProperty, "Should the state take active measures to shape public life?", [["hsl(120,70%,45%)", "hsl(0,70%,45%)", "hsl(280,60%,35%)"], ["hsl(120,70%,30%)", "hsl(0,70%,30%)", "hsl(280,60%,20%)"], ["yes", "no", "nostate"]], "Yes", q_stateFunctions, "No", q_minarchy, "The state should not exist", q_counterEcon)
}
function q_minarchy() {
...
function q_counterEcon() {
    q(q_constitution, "Which method should be used to bring down the state?", [["hsl(0,0%,50%)", "hsl(40,80%,50%)"], ["hsl(0,0%,35%)", "hsl(40,80%,35%)"], ["illegal trade", "insurrection"]], "Illegal trade", q_redMarket, "Insurrection", q_anDist)
}
...
function q_landRent() {
    q(q_anDist, "Should homesteaded property include the land it is built on?", [], "Yes", q_coop, "No", () => r(q_landRent, "Geoanarchism"))
}
function q_coop() {
...
function q_separation() {
    q(q_covenant, "How should separation of covenants occur?", [["hsl(40,100%,60%)", "hsl(25,70%,45%)"], ["hsl(40,100%,40%)", "hsl(25,70%,30%)"], ["peacefully", "aggressively"]], "Peacefully", () => r(q_separation, "Hoppeanism"), "Aggressively", () => r(q_separation, "Nilssonianism"))
}
function q_stateFunctions() {
    q(q_constitution, "Who should assume state functions?", [["hsl(220,65%,45%)", "hsl(350,70%,45%)", "hsl(45,80%,45%)"], ["hsl(220,65%,30%)", "hsl(350,70%,30%)", "hsl(45,80%,30%)"], ["elected officials", "strongman", "sovereign"]], "Elected officials", q_dist, "Strongman", q_pragmaticStrongman, "Sovereign", q_organic)
}
function q_dist() {
...
function q_safetyNet() {
    q(q_trad, "Should a social safety net protect the poor?", [], "Yes", q_deuxCentQuaranteSixFromages, "No", q_con)
}
function q_deuxCentQuaranteSixFromages() {
    q(q_safetyNet, "Who should primarily be accountable to the public?", [["hsl(260,40%,45%)", "hsl(25,50%,45%)"], ["hsl(260,40%,30%)", "hsl(25,50%,30%)"], ["state officials", "social elites",]], "State officials", () => r(q_deuxCentQuaranteSixFromages, "Dirigism"), "Social elites", () => r(q_deuxCentQuaranteSixFromages, "Paternalistic conservatism"))
}
function q_con() {
    q(q_safetyNet, "Which element is the most important for social stability?", [["hsl(240,35%,40%)", "hsl(80,60%,40%)", "hsl(10,60%,40%)"], ["hsl(250,35%,25%)", "hsl(80,60%,25%)", "hsl(10,60%,25%)"], ["institutions", "identity", "values"]], "Institutions", () => r(q_con, "Classical conservatism"), "Identity", q_identityLevel, "Values", () => r(q_con, "Liberal conservatism"))
}
function q_identityLevel() {
    q(q_con, "Which identity level should hold more power?", [["hsl(220,50%,30%)", "hsl(40,65%,40%)"], ["hsl(220,50%,20%)", "hsl(40,65%,25%)"], ["national", "regional"]], "National", () => r(q_identityLevel, "National conservatism"), "Regional", () => r(q_identityLevel, "Paleoconservatism"))
}
function q_needs() {
...
function q_regulation() {
    q(q_needs, "Should the economy be tightly regulated?", [], "Yes", q_fairness, "No", q_natLibButTheOtherKind)
}
function q_fairness() {
    q(q_regulation, "Which kind of fairness should regulation aim to achieve?", [["hsl(20,65%,45%)", "hsl(150,65%,35%)"], ["hsl(20,65%,30%)", "hsl(150,65%,20%)"], ["fair competition", "fair outcomes"]], "Fair competition", () => r(q_fairness, "Ordoliberalism"), "Fair outcomes", q_liberalLiberalLiberalLiberalHappyFunLaLaLa)
}
...
function q_natLibButTheOtherKind() {
    q(q_regulation, "Should tariffs and subsidies protect strategic interests?", [], "Yes", () => r(q_natLibButTheOtherKind, "National liberalism"), "No", q_mobility)
}
function q_mobility() {
    q(q_natLibButTheOtherKind, "Should the state subsidize job training and superior education?", [], "Yes", () => r(q_mobility, "Third way"), "No", q_hegemony)
}
function q_hegemony() {
    q(q_mobility, "What should be the primary means of generating global power?", [["hsl(350,70%,45%)", "hsl(230,70%,45%)"], ["hsl(350,70%,30%)", "hsl(230,70%,30%"], ["economic leverage", "military readiness"]], "Economic leverage", () => r(q_hegemony, "Neoliberalism"), "Military readiness", () => r(q_hegemony, "Neoconservatism"))
}
...
function q_strongmanLegit() {
    q(q_pragmaticStrongman, "Where should the strongman's authority mainly come from?", [["hsl(20,40%,30%)", "hsl(80,40%,30%)", "hsl(200,40%,30%)"], ["hsl(20,40%,15%)", "hsl(80,40%,15%)", "hsl(200,40%,15%)"], ["charisma", "armed forces", "connections"]], "Charisma", () => r(q_strongmanLegit, "Personal autocracy"), "Armed forces", () => r(q_strongmanLegit, "Stratocracy"), "Connections", () => r(q_strongmanLegit, "Patronalism"))
}
function q_racism() {
    q(q_pragmaticStrongman, "Should struggle be waged for a race superior to all others?", [], "Yes", q_naziLarp, "No", q_total)
}
function q_naziLarp() {
    q(q_racism, "How should the struggle for the race manifest itself?", [["hsl(30,50%,35%)", "hsl(345,50%,35%)", "hsl(60,50%,35%)"], ["hsl(30,50%,20%)", "hsl(345,50%,20%)", "hsl(60,50%,20%)"], ["politics", "guerrilla", "terrorism"]], "Politics", q_artaman, "Guerrilla", () => r(q_naziLarp, "Nazi maoism"), "Terrorism", () => r(q_naziLarp, "Siegism"))
}
function q_artaman() {
    q(q_naziLarp, "Should rural life be actively promoted to strengthen the race?", [], "Yes", () => r(q_artaman, "Agrarian nazism"), "No", q_raceLarp)
}
function q_raceLarp() {
    q(q_artaman, "What gives the race its superiority?", [["hsl(190,65%,35%)", "hsl(315,50%,35%)"], ["hsl(190,65%,20%)", "hsl(315,50%,20%)"], ["biology", "spirits"]], "Biology", () => r(q_raceLarp, "National socialism"), "Spirits", () => r(q_raceLarp, "Esoteric nazism"))
}
function q_total() {
    q(q_racism, "Should the state have a role in all aspects of society?", [], "Yes", q_palingenesis, "No", q_corpo)
}
function q_palingenesis() {
    q(q_total, "Should the nation be secured through a rebirth or revival?", [], "Yes", q_antiSpaghetti, "No", q_castes)
}
...
function q_control() {
    q(q_castes, "How should control over society be ensured?", [["hsl(320,50%,65%)", "hsl(0,70%,40%)"], ["hsl(320,50%,55%)", "hsl(0,70%,25%)"], ["apathy", "terror"]], "Apathy", () => r(q_control, "Fordism"), "Terror", () => r(q_control, "English socialism"))
}
function q_corpo() {
    q(q_total, "Should profession groups partake in policy making?", [], "Yes", q_corpoFocus, "No", q_natDist)
}
function q_corpoFocus() {
    q(q_corpo, "Whose interests should hold primacy during bargaining?", [["hsl(200,65%,45%)", "hsl(350,65%,45%)", "hsl(45,70%,45%)"], ["hsl(200,65%,30%)", "hsl(350,65%,30%)", "hsl(45,70%,30%)"], ["state", "labor", "business"]], "State", () => r(q_corpoFocus, "State corporatism"), "Labor", () => r(q_corpoFocus, "Yellow socialism"), "Business", () => r(q_corpoFocus, "Developmentalism"))
}
function q_natDist() {
...
function q_iSwearImARealBaronAndMagicIsRealAndShit() {
    q(q_organic, "Who should assume spiritual functions?", [["hsl(210,65%,35%)", "hsl(350,70%,30%)"], ["hsl(210,65%,25%)", "hsl(350,70%,20%)"], ["clerics", "warriors"]], "Clerics", () => r(q_iSwearImARealBaronAndMagicIsRealAndShit, "Integralism"), "Warriors", () => r(q_iSwearImARealBaronAndMagicIsRealAndShit, "Superfascism"))
}
function q_sovereignType() {
    q(q_organic, "Where should the sovereign's legitimacy come from?", [["hsl(230,70%,60%)", "hsl(70,70%,45%)", "hsl(290,70%,45%)", "hsl(35,80%,55%)", "hsl(350,70%,45%)"], ["hsl(230,60%,50%)", "hsl(70,70%,30%)", "hsl(290,70%,30%)", "hsl(35,80%,40%)", "hsl(350,70%,30%)"], ["inheritance", "wisdom", "god", "selection", "strength"]], "Inheritance", q_sovereignRole, "Wisdom", () => r(q_sovereignType, "Noocracy"), "God", q_guelph, "Selection", q_electMon, "Strength", q_weak)
}
function q_sovereignRole() {
    q(q_sovereignType, "What should be the sovereign's primary role?", [["hsl(20,45%,50%)", "hsl(270,50%,50%)", "hsl(135,45%,40%)"], ["hsl(20,45%,35%)", "hsl(270,50%,35%)", "hsl(135,45%,25%)"], ["judgment", "commandment", "management"]], "Judgment", () => r(q_sovereignRole, "Feudal monarchy"), "Commandment", () => r(q_sovereignRole, "Absolute monarchy"), "Management", () => r(q_sovereignRole, "Cameralism"))
}
function q_guelph() {
    q(q_sovereignType, "Which authority should hold primacy over the state?", [["hsl(245,35%,45%)", "hsl(85,40%,35%)"], ["hsl(245,35%,30%)", "hsl(85,40%,20%)"], ["spiritual", "temporal"]], "Spiritual", () => r(q_guelph, "Theocracy"), "Temporal", q_wifeBad)
}
function q_wifeBad() {
...
function q_electMon() {
    q(q_sovereignType, "What should grant power to select the sovereign?", [["hsl(350,60%,50%)", "hsl(25,75%,55%)", "hsl(100,50%,55%)", "hsl(230,40%,55%)"], ["hsl(350,60%,35%)", "hsl(25,75%,40%)", "hsl(100,50%,40%)", "hsl(230,40%,40%)"], ["birthright", "share holding", "land ownership", "military honors"]], "Birthright", () => r(q_electMon, "Aristocracy"), "Share holding", () => r(q_electMon, "Neocameralism"), "Land ownership", () => r(q_electMon, "Aristotelian timocracy"), "Military honors", () => r(q_electMon, "Platonic timocracy"))
}
function q_weak() {
...
function q_authMarkSoc() {
    q(q_markets, "Should the state be governed by a single, central party?", [["hsl(120,70%,45%)", "hsl(0,70%,45%)", "hsl(280,60%,35%)"], ["hsl(120,70%,30%)", "hsl(0,70%,30%)", "hsl(280,60%,20%)"], ["yes", "no", "nostate"]], "Yes", q_lange, "No", q_guilds, "The state should not exist", q_mutual)
}
function q_lange() {
...
function q_communism() {
    q(q_markets, "Should society be classless, stateless and moneyless?", [], "Yes", q_commieState, "No", q_weed)
}
...
function q_universalPPW() {
    q(q_peopleWar, "Should these tactics be attempted regardless of the country?", [], "Yes", () => r(q_universalPPW, "Marxism-leninism-maoism"), "No", q_laborAristocracy)
}
function q_laborAristocracy() {
    q(q_universalPPW, "Should first-world workers only engage in anti-imperialist actions?", [], "Yes", () => r(q_laborAristocracy, "Maoism-third-worldism"), "No", q_chingChong)
}
...
function q_songun() {
    q(q_natCom, "Should the military be given priority in resource allocation?", [], "Yes", () => r(q_songun, "Juche"), "No", () => r(q_songun, "National communism"))
}
function q_natLib() {
...
function q_reform() {
    q(q_parliament, "Should capitalism be reformed on the short term?", [], "Yes", () => r(q_reform, "Classical social democracy"), "No", () => r(q_reform, "De leonism"))
}
function q_partyDict() {
    q(q_parliament, "Should the dictatorship of the proletariat be a party dictatorship?", [], "Yes", q_nature, "No", () => r(q_partyDict, "Dutch-german left communism"))
}
function q_nature() {
    q(q_partyDict, "Should the means of production be dismantled rather than seized?", [], "Yes", () => r(q_nature, "Camattism"), "No", () => r(q_nature, "Bordigism"))
}
function q_commodity() {
    q(q_party, "Should the act of consumption be stripped from its passive elements?", [], "Yes", () => r(q_commodity, "Situationism"), "No", q_dotp)
}
function q_dotp() {
...
function q_anarchoUnions() {
    q(q_agriculture, "Should society be organized through unions?", [], "Yes", () => r(q_anarchoUnions, "Anarcho-syndicalism"), "No", q_bookchin)
}
function q_bookchin() {
...
function q_experts() {
    q(q_weed, "Should an expert committee optimize distribution to eliminate scarcity?", [], "Yes", () => r(q_experts, "Technocracy"), "No", q_hippies)
}
function q_hippies() {
    q(q_experts, "Which principle should determinate task assignment?", [["hsl(25,55%,45%)", "hsl(340,55%,45%)", "hsl(270,55%,45%)"], ["hsl(25,55%,30%)", "hsl(340,55%,30%)", "hsl(270,55%,30%)"], ["cooperation", "variety", "skill"]], "Cooperation", () => r(q_hippies, "Owenism"), "Variety", () => r(q_hippies, "Fourierism"), "Skill", () => r(q_hippies, "Saint-Simonianism"))
}
function q_transition() {
    q(q_weed, "Which method should be used to abolish capitalism?", [["hsl(325,70%,40%)", "hsl(355,70%,40%)", "hsl(25,70%,40%)"], ["hsl(325,70%,25%)", "hsl(355,70%,25%)", "hsl(25,70%,25%)"], ["election", "coup", "revolution"]], "Election", () => r(q_transition, "Democratic socialism"), "Coup", () => r(q_transition, "Blanquism"), "Revolution", q_authSoc)
}
function q_authSoc() {
...
function q_daJoos() {
    q(q_dugin, "Should inferior races be excluded from worker status and society?", [], "Yes", q_agrNazi, "No", q_natSocAuth)
}
function q_agrNazi() {
...
function q_nazbol() {
    q(q_natSynd, "How should the will of the people be executed?", [["hsl(355,70%,40%)", "hsl(325,70%,40%)", "hsl(25,70%,40%)"], ["hsl(355,70%,25%)", "hsl(325,70%,25%)", "hsl(25,70%,25%)"], ["vanguard", "parliament", "direct democracy"]], "Vanguard", () => r(q_nazbol, "National bolshevism"), "Parliament", () => r(q_nazbol, "Limonovism"), "Direct democracy", () => r(q_nazbol, "Third international theory"))
}
